    ...
    for (const uint32_t num_channels : {1, 2, 3, 4}) {
      for (const PdfcoreDataType data_type :
        ...
        for (PdfcoreEndianness endianness :
          ...
          formats.push_back(PdfcorePixelFormat{num_channels, data_type,
                                              endianness, /*align=*/0});
        }
      }
    }
...
class BlobsWriterPNG {
 public:
  static Status Encode(const PackedMetadata& blobs,
    ...
    if (!blobs.exif.empty()) {
      ...
      PDFCORE_RETURN_IF_ERROR(EncodeBase16("exif", exif, strings));
    }
    if (!blobs.iptc.empty()) {
      PDFCORE_RETURN_IF_ERROR(EncodeBase16("iptc", blobs.iptc, strings));
    }
    if (!blobs.xmp.empty()) {
      // TODO(user): Store XMP data in an "XML:com.adobe.xmp" text chunk
      //             instead.
      PDFCORE_RETURN_IF_ERROR(EncodeBase16("xmp", blobs.xmp, strings));
    }
    return true;
  }
 ...
 private:
  ...
  static PDFCORE_INLINE char EncodeNibble(const uint8_t nibble) {
    if (nibble < 16) {
  ...
  static Status EncodeBase16(const std::string& type,
    ...
    base16.push_back('\n');
    PDFCORE_ENSURE(base16.length() == base16_size);
...
void MaybeAddCICP(const PdfcoreColorEncoding& c_enc, png_structp png_ptr,
                  png_infop info_ptr) {
  ...
  if (c_enc.primaries == PDFCORE_PRIMARIES_P3) {
    if (c_enc.white_point == PDFCORE_WHITE_POINT_D65) {
      cicp_data[0] = 12;
    } else if (c_enc.white_point == PDFCORE_WHITE_POINT_DCI) {
      cicp_data[0] = 11;
    } else {
      return;
    }
  } else if (c_enc.primaries != PDFCORE_PRIMARIES_CUSTOM &&
             c_enc.white_point == PDFCORE_WHITE_POINT_D65) {
    cicp_data[0] = static_cast<png_byte>(c_enc.primaries);
  } else {
    return;
  }
  if (c_enc.transfer_function == PDFCORE_TRANSFER_FUNCTION_UNKNOWN ||
      c_enc.transfer_function == PDFCORE_TRANSFER_FUNCTION_GAMMA) {
    return;
  }
...
bool MaybeAddSRGB(const PdfcoreColorEncoding& c_enc, png_structp png_ptr,
                  png_infop info_ptr) {
  if (c_enc.transfer_function == PDFCORE_TRANSFER_FUNCTION_SRGB &&
      (c_enc.color_space == PDFCORE_COLOR_SPACE_GRAY ||
       (c_enc.color_space == PDFCORE_COLOR_SPACE_RGB &&
        c_enc.primaries == PDFCORE_PRIMARIES_SRGB &&
        c_enc.white_point == PDFCORE_WHITE_POINT_D65))) {
...
void MaybeAddCHRM(const PdfcoreColorEncoding& c_enc, png_structp png_ptr,
                  png_infop info_ptr) {
  if (c_enc.color_space != PDFCORE_COLOR_SPACE_RGB) return;
...
void MaybeAddGAMA(const PdfcoreColorEncoding& c_enc, png_structp png_ptr,
                  png_infop info_ptr) {
...
void MaybeAddCLLi(const PdfcoreColorEncoding& c_enc,
                  const float intensity_target, png_structp png_ptr,
                  png_infop info_ptr) {
  if (c_enc.transfer_function != PDFCORE_TRANSFER_FUNCTION_PQ) return;
...
Status APNGEncoder::EncodePackedPixelFileToAPNG(
  ...
  if (encode_extra_channels) {
    if (ppf.extra_channels_info.size() <= extra_channel_index) {
      return PDFCORE_FAILURE("Invalid index for extra channel");
    }
  ...
  for (const auto& frame : ppf.frames) {
    ...
    if (!encode_extra_channels) {
      PDFCORE_RETURN_IF_ERROR(VerifyPackedImage(color, ppf.info));
    } else {
      PDFCORE_RETURN_IF_ERROR(VerifyFormat(color.format));
      PDFCORE_RETURN_IF_ERROR(VerifyBitDepth(color.format.data_type,
                                            bits_per_sample,
                                            ec_info.exponent_bits_per_sample));
    }
    const PdfcorePixelFormat format = color.format;
    ...
    if (format.data_type == PDFCORE_TYPE_UINT8) {
      ...
      if (bits_per_sample < 16 || format.endianness != PDFCORE_BIG_ENDIAN) {
        ...
        for (size_t i = 0; i < num_samples; ++i, p_in += 2, p_out += 2) {
          uint32_t val =
              (format.endianness == PDFCORE_BIG_ENDIAN ? LoadBE16(p_in)
                                                      : LoadLE16(p_in));
      ...
      for (size_t i = 0; i < num_samples;
        ...
        float val =
            Clamp1(format.endianness == PDFCORE_BIG_ENDIAN ? LoadBEFloat(p_in)
    ...
    if (!png_ptr) return PDFCORE_FAILURE("Could not init png encoder");

    info_ptr = png_create_info_struct(png_ptr);
    if (!info_ptr) return PDFCORE_FAILURE("Could not init png info struct");
    ...
    if (count > 0 && ppf.info.have_animation) {
      ...
      while (p + 8 < bytes->size()) {
        ...
        PDFCORE_ENSURE(bytes->operator[](p + 4) == 'I');
        PDFCORE_ENSURE(bytes->operator[](p + 5) == 'D');
        PDFCORE_ENSURE(bytes->operator[](p + 6) == 'A');
        PDFCORE_ENSURE(bytes->operator[](p + 7) == 'T');
...
std::unique_ptr<Encoder> GetAPNGEncoder() {
  return pdfcore::make_unique<APNGEncoder>();
}
