...
StatusOr<float> SlowSeparablePixel(const ImageF& in, const Rect& rect,
                                   ...
                                   const float* PDFCORE_RESTRICT horz_weights,
                                   const float* PDFCORE_RESTRICT vert_weights) {
  ...
  for (int dy = -radius; dy <= radius; ++dy) {
    ...
    PDFCORE_ENSURE(sy < ysize);
    const float* const PDFCORE_RESTRICT row = in.ConstRow(sy);
    for (int dx = -radius; dx <= radius; ++dx) {
      const float wx = horz_weights[std::abs(dx) * 4];
      const size_t sx = wrap(rect.x0() + x + dx, xsize);
      PDFCORE_ENSURE(sx < xsize);
      mul += row[sx] * wx * wy;
    }
  }
  return mul;
...
Status SlowSeparable(const ImageF& in, const Rect& in_rect,
                     ...
                     const Rect& out_rect) {
  PDFCORE_ENSURE(in_rect.xsize() == out_rect.xsize());
  PDFCORE_ENSURE(in_rect.ysize() == out_rect.ysize());
  PDFCORE_ENSURE(in_rect.IsInside(Rect(in)));
  PDFCORE_ENSURE(out_rect.IsInside(Rect(*out)));
  ...
  const auto process_row = [&](const uint32_t task,
    ...
    float* const PDFCORE_RESTRICT row_out = out_rect.Row(out, y);
