...
class Separable5Impl {
 public:
  ...
  Status Run() {
    ...
    if (rect.xsize() >= min_width) {
      PDFCORE_ENSURE(SameSize(rect, *out));
  ...
  PDFCORE_NOINLINE void ConvolveRow(const uint32_t y) {
    const D d;
    const int64_t stride = in->PixelsPerRow();
    const int64_t neg_stride = -stride;  // allows LEA addressing.
    const size_t xsize = rect.xsize();
    const float* const PDFCORE_RESTRICT row_m = rect.ConstRow(*in, y);
    float* const PDFCORE_RESTRICT row_out = out->Row(y);
    const float* PDFCORE_RESTRICT row_t2 = row_m + 2 * neg_stride;
    const float* PDFCORE_RESTRICT row_t1 = row_m + 1 * neg_stride;
    const float* PDFCORE_RESTRICT row_b1 = row_m + 1 * stride;
    const float* PDFCORE_RESTRICT row_b2 = row_m + 2 * stride;
    ...
    if (kBorder) {
      ...
      if (in->ysize() <= 2 * kRadius) {  // Very special: double reflections
        ...
        if (img_y == 0) {
          ...
          PDFCORE_DASSERT(img_y == 1);
          row_t2 = row_t1;
        }
      } else {
        PDFCORE_DASSERT(img_y + kRadius >= in->ysize());
        if (img_y + 1 == in->ysize()) {
          row_b1 = row_m;
          row_b2 = row_t1;
        } else {
          PDFCORE_DASSERT(img_y + 2 == in->ysize());
          row_b2 = row_b1;
        }
      }
    }
    ...
    if (kSizeModN != 0) {
      const float* PDFCORE_RESTRICT rows[5] = {row_t2, row_t1, row_m, row_b1,
                                              row_b2};
      for (; x < xsize; ++x) {
        float mul = 0.0f;
 ...
 private:
  ...
  template <size_t kSizeModN>
  PDFCORE_INLINE void RunBorderRows(const size_t ybegin, const size_t yend) {
  ...
  template <size_t kSizeModN>
  PDFCORE_INLINE void RunInteriorRows(const size_t ybegin, const size_t yend) {
  ...
  template <size_t M>
  static PDFCORE_INLINE I MirrorLanes() {
  ...
  static PDFCORE_MAYBE_INLINE V HorzConvolveFirst(
      const float* const PDFCORE_RESTRICT row, const int64_t x,
  ...
  template <size_t kSizeModN>
  static PDFCORE_MAYBE_INLINE V
  HorzConvolveLast(const float* const PDFCORE_RESTRICT row, const int64_t x,
                   const int64_t xsize, const V wh0, const V wh1, const V wh2,
  ...
  static PDFCORE_MAYBE_INLINE V
  HorzConvolve(const float* const PDFCORE_RESTRICT pos, const V wh0, const V wh1,
               const V wh2) {
    const D d;
