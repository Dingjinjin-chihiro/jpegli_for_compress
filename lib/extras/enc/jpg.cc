...
Status SetJpegProgression(int progressive_id,
  ...
  if (progressive_id > static_cast<int>(kNumScans.size())) {
    return PDFCORE_FAILURE("Unknown jpeg scan script id %d", progressive_id);
  }
...
Status EncodeWithLibJpeg(const PackedImage& image, const PdfcoreBasicInfo& info,
                         const std::vector<uint8_t>& icc,
                         std::vector<uint8_t> exif, const JpegParams& params,
                         std::vector<uint8_t>* bytes) {
  if (BITS_IN_JSAMPLE != 8 || sizeof(JSAMPLE) != 1) {
    return PDFCORE_FAILURE("Only 8 bit JSAMPLE is supported.");
  }
  jpeg_compress_struct cinfo = {};
  jpeg_error_mgr jerr;
...
Status EncodeWithSJpeg(const PackedImage& image, const PdfcoreBasicInfo& info,
                       const std::vector<uint8_t>& icc,
                       std::vector<uint8_t> exif, const JpegParams& params,
                       std::vector<uint8_t>* bytes) {
  return PDFCORE_FAILURE("JPEGLI was built without sjpeg support");
}

#else  //  PDFCORE_ENABLE_SJPEG
...
Status EncodeWithSJpeg(const PackedImage& image, const PdfcoreBasicInfo& info,
  ...
  if (image.format.data_type != PDFCORE_TYPE_UINT8) {
    return PDFCORE_FAILURE("Unsupported pixel data type");
  }
  if (info.alpha_bits > 0) {
    return PDFCORE_FAILURE("alpha is not supported");
  }
  ...
  if (params.libjpeg_quality > 0) {
    ...
    PDFCORE_RETURN_IF_ERROR(EncodeWithLibJpeg(image, info, icc, exif,
  ...
  if (param.target_mode != sjpeg::EncoderParam::TARGET_NONE) {
    ...
    hook = pdfcore::make_unique<MySearchHook>();
...
#endif  // PDFCORE_ENABLE_SJPEG

Status EncodeImageJPG(const PackedImage& image, const PdfcoreBasicInfo& info,
                      const std::vector<uint8_t>& icc,
                      std::vector<uint8_t> exif, JpegEncoder encoder,
                      const JpegParams& params, ThreadPool* pool,
                      std::vector<uint8_t>* bytes) {
  if (params.quality > 100) {
    return PDFCORE_FAILURE("please specify a 0-100 JPEG quality");
  }
  ...
  switch (encoder) {
    ...
    case JpegEncoder::kSJpeg:
      ...
      break;
    default:
      return PDFCORE_FAILURE("tried to use an unknown JPEG encoder");
  }
  ...
  Status Encode(const PackedPixelFile& ppf, EncodedImage* encoded_image,
    ...
    for (const auto& it : options()) {
      if (it.first == "q") {
        ...
        PDFCORE_RETURN_IF_ERROR(static_cast<bool>(is >> params.quality));
      } else if (it.first == "libjpeg_quality") {
        std::istringstream is(it.second);
        PDFCORE_RETURN_IF_ERROR(static_cast<bool>(is >> params.libjpeg_quality));
        ...
        if (it.second == "libjpeg") {
          ...
          return PDFCORE_FAILURE("unknown jpeg encoder \"%s\"",
                                it.second.c_str());
        }
      } else if (it.first == "progressive") {
        std::istringstream is(it.second);
        PDFCORE_RETURN_IF_ERROR(static_cast<bool>(is >> params.progressive_id));
    ...
    params.is_xyb = (ppf.color_encoding.color_space == PDFCORE_COLOR_SPACE_XYB);
    ...
    for (const auto& frame : ppf.frames) {
      ...
      PDFCORE_RETURN_IF_ERROR(EncodeImageJPG(
...
std::unique_ptr<Encoder> GetJPEGEncoder() {
  return pdfcore::make_unique<JPEGEncoder>();
}
