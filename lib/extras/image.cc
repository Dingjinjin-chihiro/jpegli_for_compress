...
void InitializePadding(PlaneBase& plane, const size_t sizeof_t) {
  ...
  for (size_t y = 0; y < ysize; ++y) {
    uint8_t* PDFCORE_RESTRICT row = plane.bytes() + y * plane.bytes_per_row();
...
Status PlaneBase::Allocate(PdfcoreMemoryManager* memory_manager,
                           size_t pre_padding) {
  PDFCORE_ENSURE(bytes_.address<void>() == nullptr);
