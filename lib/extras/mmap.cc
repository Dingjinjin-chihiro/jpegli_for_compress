...
struct MemoryMappedFileImpl {
  static StatusOr<std::unique_ptr<MemoryMappedFileImpl>> Init(
    ...
    if (f->fd == -1) {
      return PDFCORE_FAILURE("Cannot open file %s", path);
    }
    ...
    if (f->ptr == MAP_FAILED) {
      return PDFCORE_FAILURE("mmap failure");
    }
    return f;
  }
...
struct MemoryMappedFileImpl {
  static StatusOr<std::unique_ptr<MemoryMappedFileImpl>> Init(
    ...
    if (!GetFileSizeEx(f->handle.get(), &f->fsize)) {
      return PDFCORE_FAILURE("Cannot get file size (%s)", path);
    }
    f->handle_mapping.reset(CreateFileMappingW(f->handle.get(), nullptr,
                                               PAGE_READONLY, 0, 0, nullptr));
    if (f->handle_mapping == nullptr) {
      return PDFCORE_FAILURE("Cannot create memory mapping (%s)", path);
    }
...
struct MemoryMappedFileImpl {
  static StatusOr<std::unique_ptr<MemoryMappedFileImpl>> Init(
      const char* path) {
    return PDFCORE_FAILURE("Memory mapping not supported on this system");
  }
...
StatusOr<MemoryMappedFile> MemoryMappedFile::Init(const char* path) {
  PDFCORE_ASSIGN_OR_RETURN(auto mmf, MemoryMappedFileImpl::Init(path));
