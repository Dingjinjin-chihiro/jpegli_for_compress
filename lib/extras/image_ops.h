...
#ifndef PDFCORE_LIB_EXTRAS_IMAGE_OPS_H_
#define PDFCORE_LIB_EXTRAS_IMAGE_OPS_H_
...
template <typename T>
Status CopyImageTo(const Plane<T>& from, Plane<T>* PDFCORE_RESTRICT to) {
  PDFCORE_ENSURE(SameSize(from, *to));
  if (from.ysize() == 0 || from.xsize() == 0) return true;
  for (size_t y = 0; y < from.ysize(); ++y) {
    const T* PDFCORE_RESTRICT row_from = from.ConstRow(y);
    T* PDFCORE_RESTRICT row_to = to->Row(y);
...
Status CopyImageTo(const Rect& rect_from, const Plane<T>& from,
                   const Rect& rect_to, Plane<T>* PDFCORE_RESTRICT to) {
  PDFCORE_ENSURE(SameSize(rect_from, rect_to));
  PDFCORE_ENSURE(rect_from.IsInside(from));
  PDFCORE_ENSURE(rect_to.IsInside(*to));
  if (rect_from.xsize() == 0) return true;
  for (size_t y = 0; y < rect_from.ysize(); ++y) {
    const T* PDFCORE_RESTRICT row_from = rect_from.ConstRow(from, y);
    T* PDFCORE_RESTRICT row_to = rect_to.Row(to, y);
...
Status CopyImageTo(const Rect& rect_from, const Image3<T>& from,
                   const Rect& rect_to, Image3<T>* PDFCORE_RESTRICT to) {
  PDFCORE_ENSURE(SameSize(rect_from, rect_to));
...
Status ConvertPlaneAndClamp(const Rect& rect_from, const Plane<T>& from,
                            const Rect& rect_to, Plane<U>* PDFCORE_RESTRICT to) {
  PDFCORE_ENSURE(SameSize(rect_from, rect_to));
  using M = decltype(T() + U());
  for (size_t y = 0; y < rect_to.ysize(); ++y) {
    const T* PDFCORE_RESTRICT row_from = rect_from.ConstRow(from, y);
    U* PDFCORE_RESTRICT row_to = rect_to.Row(to, y);
...
// Copies `from` to `to`.
template <typename T>
Status CopyImageTo(const T& from, T* PDFCORE_RESTRICT to) {
...
Status CopyImageToWithPadding(const Rect& from_rect, const T& from,
  ...
  PDFCORE_ENSURE(to_rect.x0() >= xextra0);
  PDFCORE_ENSURE(to_rect.y0() >= yextra0);
...
StatusOr<Plane<T>> LinComb(const T lambda1, const Plane<T>& image1,
  ...
  PDFCORE_ENSURE(xsize == image2.xsize());
  PDFCORE_ENSURE(ysize == image2.ysize());
  PdfcoreMemoryManager* memory_manager = image1.memory_manager();
  PDFCORE_ASSIGN_OR_RETURN(Plane<T> out,
                          Plane<T>::Create(memory_manager, xsize, ysize));
  for (size_t y = 0; y < ysize; ++y) {
    const T* const PDFCORE_RESTRICT row1 = image1.Row(y);
    const T* const PDFCORE_RESTRICT row2 = image2.Row(y);
    T* const PDFCORE_RESTRICT row_out = out.Row(y);
...
void ScaleImage(const T lambda, Plane<T>* image) {
  for (size_t y = 0; y < image->ysize(); ++y) {
    T* const PDFCORE_RESTRICT row = image->Row(y);
...
struct WrapMirror {
  PDFCORE_INLINE int64_t operator()(const int64_t coord,
...
template <typename T>
void ImageMinMax(const Plane<T>& image, T* const PDFCORE_RESTRICT min,
                 T* const PDFCORE_RESTRICT max) {
  *min = std::numeric_limits<T>::max();
  *max = std::numeric_limits<T>::lowest();
  for (size_t y = 0; y < image.ysize(); ++y) {
    const T* const PDFCORE_RESTRICT row = image.Row(y);
    for (size_t x = 0; x < image.xsize(); ++x) {
...
void FillImage(const T value, Image3<T>* image) {
  for (size_t c = 0; c < 3; ++c) {
    for (size_t y = 0; y < image->ysize(); ++y) {
      T* PDFCORE_RESTRICT row = image->PlaneRow(c, y);
      for (size_t x = 0; x < image->xsize(); ++x) {
        row[x] = value;
      }
    }
...
void FillPlane(const T value, Plane<T>* image, Rect rect) {
  for (size_t y = 0; y < rect.ysize(); ++y) {
    T* PDFCORE_RESTRICT row = rect.Row(image, y);
...
#endif  // PDFCORE_LIB_EXTRAS_IMAGE_OPS_H_
