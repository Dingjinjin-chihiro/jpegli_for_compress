...
#ifndef PDFCORE_LIB_EXTRAS_COLOR_HINTS_H_
#define PDFCORE_LIB_EXTRAS_COLOR_HINTS_H_
 ...
 public:
  ...
  Status Foreach(const Func& func) const {
    for (const KeyValue& kv : kv_) {
      ...
      if (!ok) {
        return PDFCORE_FAILURE("ColorHints::Foreach returned false");
      }
    }
    return true;
  }
...
#endif  // PDFCORE_LIB_EXTRAS_COLOR_HINTS_H_
