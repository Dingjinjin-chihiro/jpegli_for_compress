...
void DequantBlock(const int16_t* PDFCORE_RESTRICT qblock,
                  const float* PDFCORE_RESTRICT dequant,
                  const float* PDFCORE_RESTRICT biases,
                  float* PDFCORE_RESTRICT block) {
...
template <size_t N>
void ForwardEvenOdd(const float* PDFCORE_RESTRICT a_in, size_t a_in_stride,
...
template <size_t N>
void BTranspose(float* PDFCORE_RESTRICT coeff) {
...
template <size_t N>
void MultiplyAndAdd(const float* PDFCORE_RESTRICT coeff,
                    float* PDFCORE_RESTRICT out, size_t out_stride) {
  for (size_t i = 0; i < N / 2; i++) {
...
template <>
struct IDCT1DImpl<1> {
  PDFCORE_INLINE void operator()(const float* from, size_t from_stride,
...
struct IDCT1DImpl<2> {
  PDFCORE_INLINE void operator()(const float* from, size_t from_stride,
                                float* to, size_t to_stride) {
    PDFCORE_DASSERT(from_stride >= 8);
    PDFCORE_DASSERT(to_stride >= 8);
...
template <size_t N>
void IDCT1D(float* PDFCORE_RESTRICT from, float* PDFCORE_RESTRICT output,
...
void ComputeScaledIDCT(float* PDFCORE_RESTRICT block0,
                       float* PDFCORE_RESTRICT block1,
                       float* PDFCORE_RESTRICT output, size_t output_stride) {
...
void InverseTransformBlock8x8(const int16_t* PDFCORE_RESTRICT qblock,
                              const float* PDFCORE_RESTRICT dequant,
                              const float* PDFCORE_RESTRICT biases,
                              float* PDFCORE_RESTRICT scratch_space,
                              float* PDFCORE_RESTRICT output,
                              size_t output_stride, size_t dctsize) {
  float* PDFCORE_RESTRICT block0 = scratch_space;
  float* PDFCORE_RESTRICT block1 = scratch_space + DCTSIZE2;
...
void InverseTransformBlockGeneric(const int16_t* PDFCORE_RESTRICT qblock,
                                  const float* PDFCORE_RESTRICT dequant,
                                  const float* PDFCORE_RESTRICT biases,
                                  float* PDFCORE_RESTRICT scratch_space,
                                  float* PDFCORE_RESTRICT output,
  ...
  if (dctsize == 1) {
    *output = *block0;
    ...
    float* PDFCORE_RESTRICT block2 = scratch_space + 2 * DCTSIZE2;
...
pdfcore::Status ChooseInverseTransform(j_decompress_ptr cinfo) {
  ...
  for (int c = 0; c < cinfo->num_components; ++c) {
    ...
    if (dct_size < 1 || dct_size > 16) {
      return PDFCORE_FAILURE("Compute1dIDCT does not support N=%d", dct_size);
    }
    if (dct_size == DCTSIZE) {
