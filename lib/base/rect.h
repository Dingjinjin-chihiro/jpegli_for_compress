...
#ifndef PDFCORE_LIB_BASE_RECT_H_
#define PDFCORE_LIB_BASE_RECT_H_
...
class RectT {
 public:
  ...
  PDFCORE_MUST_USE_RESULT RectT Intersection(const RectT& other) const {
    return RectT(std::max(x0_, other.x0_), std::max(y0_, other.y0_), xsize_,
                 ysize_, std::min(x1(), other.x1()),
                 std::min(y1(), other.y1()));
  }

  PDFCORE_MUST_USE_RESULT RectT Translate(int64_t x_offset,
  ...
  StatusOr<RectT<T>> CeilShiftRight(std::pair<size_t, size_t> shift) const {
    ...
    PDFCORE_ENSURE((x0_ % (1 << shiftx) == 0) && (y0_ % (1 << shifty) == 0));
...
#endif  // PDFCORE_LIB_BASE_RECT_H_
