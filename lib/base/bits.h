...
#ifndef PDFCORE_LIB_BASE_BITS_H_
#define PDFCORE_LIB_BASE_BITS_H_
...
static PDFCORE_INLINE PDFCORE_MAYBE_UNUSED size_t
...
template <typename T>
static PDFCORE_INLINE PDFCORE_MAYBE_UNUSED size_t Num0BitsAboveMS1Bit(const T x) {
  return (x == 0) ? sizeof(T) * 8 : Num0BitsAboveMS1Bit_Nonzero(x);
}

// Returns bit width for x == 0.
template <typename T>
static PDFCORE_INLINE PDFCORE_MAYBE_UNUSED size_t Num0BitsBelowLS1Bit(const T x) {
  return (x == 0) ? sizeof(T) * 8 : Num0BitsBelowLS1Bit_Nonzero(x);
}

// Returns base-2 logarithm, rounded down.
template <typename T>
static PDFCORE_INLINE PDFCORE_MAYBE_UNUSED size_t FloorLog2Nonzero(const T x) {
  return (sizeof(T) * 8 - 1) ^ Num0BitsAboveMS1Bit_Nonzero(x);
}

// Returns base-2 logarithm, rounded up.
template <typename T>
static PDFCORE_INLINE PDFCORE_MAYBE_UNUSED size_t CeilLog2Nonzero(const T x) {
...
#endif  // PDFCORE_LIB_BASE_BITS_H_
