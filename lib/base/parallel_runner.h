...
/** @addtogroup libpdfcore_jpegli_threads
 *  @{
 */
/**
 ...
 * assigning threads.
 *
 * The PdfcoreParallelRunner function type defines a parallel data processing
 ...
 * jpegli/parallel_runner_thread.h (part of the pdfcore_jpegli_threads library).
 *
 ...
 *
 * If no multi-threading is desired, a @c NULL value of PdfcoreParallelRunner
 * will use an internal implementation without multi-threading.
 */

#ifndef PDFCORE_PARALLEL_RUNNER_H_
#define PDFCORE_PARALLEL_RUNNER_H_
...
/** Return code used in the PdfcoreParallel* functions as return value. A value
 * of ::PDFCORE_PARALLEL_RET_SUCCESS means success and any other value means
 * error. The special value ::PDFCORE_PARALLEL_RET_RUNNER_ERROR can be used by
 * the runner to indicate any other error.
 */
typedef int PdfcoreParallelRetCode;

/**
 * Code returned by the @ref PdfcoreParallelRunInit function to indicate success.
 */
#define PDFCORE_PARALLEL_RET_SUCCESS (0)

/**
 * Code returned by the @ref PdfcoreParallelRunInit function to indicate a
 * general error.
 */
#define PDFCORE_PARALLEL_RET_RUNNER_ERROR (-1)

/**
 * Parallel run initialization callback. See @ref PdfcoreParallelRunner for
 * details.
 *
 * This function MUST be called by the PdfcoreParallelRunner only once, on the
 * same thread that called @ref PdfcoreParallelRunner, before any parallel
 * execution. The purpose of this call is to provide the maximum number of
 * threads that the
 * @ref PdfcoreParallelRunner will use, which can be used by JPEGLI to allocate
 * per-thread storage if needed.
 *
 * @param pdfcore_jpegli_opaque the @p pdfcore_jpegli_opaque handle provided to
 * @ref PdfcoreParallelRunner() must be passed here.
 ...
 * @ref PdfcoreParallelRunner().
 */
typedef PdfcoreParallelRetCode (*PdfcoreParallelRunInit)(void* pdfcore_jpegli_opaque,
                                                       size_t num_threads);

/**
 * Parallel run data processing callback. See @ref PdfcoreParallelRunner for
 * details.
 *
 ...
 * @ref PdfcoreParallelRunInit.
 */
typedef void (*PdfcoreParallelRunFunction)(void* pdfcore_jpegli_opaque, uint32_t value,
                                          size_t thread_id);

/**
 * PdfcoreParallelRunner function type. A parallel runner implementation can be
 ...
 *
 * The @ref PdfcoreParallelRunner function does not need to be re-entrant. This
 * means that the same @ref PdfcoreParallelRunner function with the same
 ...
 * or encoding instance may call the provided @ref PdfcoreParallelRunner multiple
 ...
 * @return PDFCORE_PARALLEL_RET_RUNNER_ERROR if an error occurred in the runner
 * code, for example, setting up the threads.
 * @return the return value of @p init() if non-zero.
 */
typedef PdfcoreParallelRetCode (*PdfcoreParallelRunner)(
...
/* The following is an example of a @ref PdfcoreParallelRunner that doesn't use
 any
...
#endif /* PDFCORE_PARALLEL_RUNNER_H_ */
