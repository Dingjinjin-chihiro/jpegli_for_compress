...
#ifndef PDFCORE_LIB_CMS_TRANSFER_FUNCTIONS_H_
#define PDFCORE_LIB_CMS_TRANSFER_FUNCTIONS_H_
...
class TF_PQ_Base {
 public:
  static double DisplayFromEncoded(float display_intensity_target, double e) {
    ...
    PDFCORE_DASSERT(den != 0.0);
    const double d = std::pow(num / den, 1.0 / kM1);
    PDFCORE_DASSERT(d >= 0.0);  // Equal for e ~= 1E-9
...
#endif  // PDFCORE_LIB_CMS_TRANSFER_FUNCTIONS_H_
