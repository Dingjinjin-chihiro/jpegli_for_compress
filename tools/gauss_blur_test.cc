...
bool VerifyRelativeError(const Plane<T>& expected, const Plane<T>& actual,
                         ...
                         const int c = 0) {
  PDFCORE_ENSURE(SameSize(expected, actual));
  ...
  for (intptr_t y = border; y < ysize - border; ++y) {
    const T* const PDFCORE_RESTRICT row_expected = expected.Row(y);
    const T* const PDFCORE_RESTRICT row_actual = actual.Row(y);
...
PDFCORE_INLINE void RandomFillImage(Plane<float>* image) {
  Rng rng(129);
...
PDFCORE_INLINE void RandomFillImage(Image3F* image) {
  Rng rng(129);
  GenerateImage(rng, image, 0.0f, std::numeric_limits<float>::max());
}
...
void ExtrapolateBorders(const float* const PDFCORE_RESTRICT row_in,
                        float* const PDFCORE_RESTRICT row_out, const int xsize,
                        const int radius) {
...
ImageF ConvolveXAndTranspose(const ImageF& in,
  ...
  float* const PDFCORE_RESTRICT rowp = &row_tmp[r];
...
void TestDirac2D(size_t xsize, size_t ysize, double sigma) {
  PdfcoreMemoryManager* memory_manager = pdfcore::test::MemoryManager();
  PDFCORE_TEST_ASSIGN_OR_DIE(ImageF in,
                            ImageF::Create(memory_manager, xsize, ysize));
  ZeroFillImage(&in);
  // We anyway ignore the border below, so might as well choose the middle.
  in.Row(ysize / 2)[xsize / 2] = 1.0f;

  PDFCORE_TEST_ASSIGN_OR_DIE(ImageF temp,
                            ImageF::Create(memory_manager, xsize, ysize));
  PDFCORE_TEST_ASSIGN_OR_DIE(ImageF out,
                            ImageF::Create(memory_manager, xsize, ysize));
...
TEST(GaussBlurTest, DISABLED_SlowTestDirac1D) {
  ...
  PDFCORE_TEST_ASSIGN_OR_DIE(ImageF inputs,
...
TEST(GaussBlurTest, TestSign) {
  ...
  PDFCORE_TEST_ASSIGN_OR_DIE(ImageF temp,
                            ImageF::Create(memory_manager, xsize, ysize));
  PDFCORE_TEST_ASSIGN_OR_DIE(ImageF out_rg,
                            ImageF::Create(memory_manager, xsize, ysize));
  const auto rg = CreateRecursiveGaussian(sigma);
  ASSERT_TRUE(FastGaussian(
      memory_manager, rg, in.xsize(), in.ysize(),
